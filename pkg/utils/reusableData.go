...
func GetRoutingData(ipaddress string, s []RoutingTablesUtils) (map[string][]string,[]string,string) {
	...
	for i:= range s {
		if (s[i].Ipaddress == ipaddress) {
			fmt.Println("data:",s[i].RoutingEntries, s[i].RoutingTables,s[i].Time)
